...
const PollMessage = ({ pollData, messageId, currentUserId, channel }) => {
  ...
  // Parse poll data if it's a string
  ...
  useEffect(() => {
    ...
    // Check if current user has already voted (from localStorage for demo)
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      // Save vote locally (in production, this would be saved to backend)
      const voteKey = `poll_${messageId}_${currentUserId}`;
      localStorage.setItem(voteKey, JSON.stringify(Array.from(selectedOptions)));

      // In production: Update the message in Stream Chat
      // await channel.updateMessage({
      //   id: messageId,
      //   attachments: [{
      //     ...message.attachments[0],
      //     poll_data: JSON.stringify({ ...pollState, options: updatedOptions })
      //   }]
      // });
